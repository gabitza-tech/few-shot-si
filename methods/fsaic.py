...
class FSAIC():
    ...
    def calculate_mahalanobis_centroids(self, enroll_embs, test_embs, enroll_labels, method='centroid'):
        ...
        elif method == 'unnormalized_normal_mahalanobis': # we do not use the normalized centroids!
            diff_sq_support = np.sum(z_s - w_sq_all,axis=2,keepdims=True)
            diff_s_support = np.sum(z_s - w_s_all,axis=2,keepdims=True)
            diff_sq_query = np.sum(test_embs - w_sq_all,axis=2,keepdims=True)
        ...
        elif method == 'unnormalized_centroid_mahalanobis':
            diff_sq_support = np.sum(z_s - w_sq_all,axis=2,keepdims=True)
            diff_s_support = np.sum(z_s - w_s_all,axis=2,keepdims=True)
            diff_sq_query = np.sum(w_q_all - w_sq_all,axis=2,keepdims=True)
        ...
        elif method == 'mahalanobis_cd':
            
            diff_s_query = np.sum(w_q - w_s,axis=2,keepdims=True)
            
            dist_w_s_query = np.sum(np.einsum('...ij,...jk->...ik', diff_s_query, inv_cov_matrices) * diff_s_query, axis=-1)

            final_distance = dist_w_s_query
            return final_distance
        
        elif method == 'unnormalized_mahalanobis_cd':
            
            diff_s_query = np.sum(w_q_all - w_s_all,axis=2,keepdims=True)
